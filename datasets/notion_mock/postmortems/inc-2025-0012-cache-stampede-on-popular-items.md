# Postmortem: INC-2025-0012 — Cache Stampede on Popular Items

**Date Written:** 2025-03-21
**Incident Date:** 2025-01-20 18:44–19:18 UTC
**Duration:** 34 minutes
**Severity:** P1
**DRI:** Database Team
...
cache stampede on popular items occurred on 2025-01-20. 34 minutes of database spikes, P99 query latency 5.2 seconds, cache hit rate dropped to 2%. The incident was caused by popular item cache keys expired simultaneously. 500+ concurrent requests hit db for same key. no probabilistic early expiration.. The service was recovered through implemented cache stampede mitigation (probabilistic early expiration), added cache key warming.

## Timeline

| Time (UTC) | Actor | Action | Result |
|---|---|---|---|
| 18:44 | Monitoring System | Alert fired | Service metric exceeded threshold |
| 18:47 | On-Call Engineer | Paged | Notification received, joined war room |
| 18:52 | Platform Engineer | Investigated | Root cause identified from logs and metrics |
| 18:56 | Senior Engineer | Remediation deployed | Applied fixes and verified recovery |
| 19:18 | Monitoring System | Service recovered | Metrics returned to baseline |

## Root Cause Analysis (5 Whys)

1. Why did cache stampede on popular items occur?
   → Popular item cache keys expired simultaneously. 500+ concurrent requests hit DB for same key. No probabilistic early expiration.
...
- Implemented cache stampede mitigation (probabilistic early expiration), added cache key warming
...
|---|---|---|---|
| A1 | Implement monitoring/alerting to catch recurrence | Database Team | 2025-01-27 |
| A2 | Add automated safeguards (circuit breaker/fallback) | Database Team | 2025-02-03 |
| A3 | Schedule architecture review and update runbook | Database Team | 2025-02-10 |
