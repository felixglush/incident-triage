# Postmortem: INC-2025-0044 — Fraud Service Timeout Cascade

**Date Written:** 2025-03-21
**Incident Date:** 2025-02-03 16:24–16:35 UTC
**Duration:** 11 minutes
**Severity:** P1
...
fraud service timeout cascade occurred on 2025-02-03. Checkout success rate dropped to 6%, 8,900 failed attempts, ~$1.6M GMV lost. The incident was caused by fraud-service ml model update caused 30-second cold start. checkout-service had no timeout on fraud-service rpc, goroutines piled up, exhausted connection pool.. The service was recovered through rolled back fraud-service, added timeout to checkout-service fraud-service calls, improved startup probes.

## Timeline

| Time (UTC) | Actor | Action | Result |
|---|---|---|---|
| 16:24 | Monitoring System | Alert fired | Service metric exceeded threshold |
| 16:27 | On-Call Engineer | Paged | Notification received, joined war room |
| 16:32 | Platform Engineer | Investigated | Root cause identified from logs and metrics |
| 16:36 | Senior Engineer | Remediation deployed | Applied fixes and verified recovery |
| 16:35 | Monitoring System | Service recovered | Metrics returned to baseline |

## Root Cause Analysis (5 Whys)

1. Why did fraud service timeout cascade occur?
   → Fraud-service ML model update caused 30-second cold start. Checkout-service had no timeout on fraud-service RPC, goroutines piled up, exhausted connection pool.
...
- Rolled back fraud-service, added timeout to checkout-service fraud-service calls, improved startup probes
...
|---|---|---|---|
| A1 | Implement monitoring/alerting to catch recurrence | Payments Team | 2025-02-10 |
| A2 | Add automated safeguards (circuit breaker/fallback) | Payments Team | 2025-02-17 |
| A3 | Schedule architecture review and update runbook | Payments Team | 2025-02-24 |
