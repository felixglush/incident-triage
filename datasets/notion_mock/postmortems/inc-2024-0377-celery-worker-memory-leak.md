# Postmortem: INC-2024-0377 — Celery Worker Memory Leak

**Date Written:** 2025-03-21
**Incident Date:** 2024-11-12 19:40–20:18 UTC
**Duration:** 38 minutes
**Severity:** P1
...
celery worker memory leak occurred on 2024-11-12. Worker processes OOM-killed during high-volume processing, 38 minutes of degraded throughput, order lag accumulated. The incident was caused by worker code had memory leak in task result processing. holding large result objects in memory without cleanup.. The service was recovered through deployed memory-efficient result handling, added memory monitoring, configured proactive worker restart.

## Timeline

| Time (UTC) | Actor | Action | Result |
|---|---|---|---|
| 19:40 | Monitoring System | Alert fired | Service metric exceeded threshold |
| 19:43 | On-Call Engineer | Paged | Notification received, joined war room |
| 19:48 | Platform Engineer | Investigated | Root cause identified from logs and metrics |
| 19:52 | Senior Engineer | Remediation deployed | Applied fixes and verified recovery |
| 20:18 | Monitoring System | Service recovered | Metrics returned to baseline |

## Root Cause Analysis (5 Whys)

1. Why did celery worker memory leak occur?
   → Worker code had memory leak in task result processing. Holding large result objects in memory without cleanup.
...
- Deployed memory-efficient result handling, added memory monitoring, configured proactive worker restart
...
|---|---|---|---|
| A1 | Implement monitoring/alerting to catch recurrence | Platform Team | 2024-11-19 |
| A2 | Add automated safeguards (circuit breaker/fallback) | Platform Team | 2024-11-26 |
| A3 | Schedule architecture review and update runbook | Platform Team | 2024-12-03 |
